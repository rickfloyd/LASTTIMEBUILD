...
import { CandleData } from '../utils/patternDetection';
...
const TradingChart: React.FC<TradingChartProps> = ({ symbol = 'AAPL' }) => {
  ...
  const [dataSource, setDataSource] = useState('Backend API');
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE}/api/ohlc`, { params: { symbol } });
      const data = response.data;
      setHistoricalData(data);
      
      // Assuming the API provides a quote endpoint as well
      // const quoteResponse = await axios.get(`${API_BASE}/api/quote`, { params: { symbol } });
      // setCurrentQuote(quoteResponse.data);
  ...
  return (
    <div className="w-full h-full min-h-[600px] bg-charcoal-gradient rounded-xl border-4 border-fluorescent-pink shadow-neon-pink m-4 relative overflow-hidden">
      ...
      <div className='bg-gray-800 p-4 rounded-lg'>
        <h2 className='text-white'>Pattern Recognition</h2>
        <PatternRecognition priceData={candleData} />
      </div>
